...
bool rotation_edit(Eigen::Quaterniond& rotation, const std::string& title) {
    ...
    if (ImGui::BeginChild((title + "x").c_str(), ImVec2(width / 3, 20))) {
        ImGui::Text("roll=");
        ImGui::SameLine();
        if (ImGui::DragFloat("##roll", &eulers.x()))
            edit = true;
    }
    ImGui::EndChild();
    ImGui::SameLine();
    if (ImGui::BeginChild((title + "y").c_str(), ImVec2(width / 3, 20))) {
        ImGui::Text("pitch=");
        ImGui::SameLine();
        if (ImGui::DragFloat("##pitch", &eulers.y()))
            edit = true;
    }
    ImGui::EndChild();
    ImGui::SameLine();
    if (ImGui::BeginChild((title + "z").c_str(), ImVec2(width / 3, 20))) {
        ImGui::Text("yaw=");
        ImGui::SameLine();
        if (ImGui::DragFloat("##yaw", &eulers.z()))
            edit = true;
    }
    ImGui::EndChild();
...
bool RecordData::display() {
    ...
    if (ImGui::BeginChild(("record" + std::to_string(rand())).c_str(), ImVec2(width, height), false,
                          ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoMouseInputs)) {
        const ImVec2 container_min = ImGui::GetWindowPos();
        const ImVec2 container_max = ImVec2(container_min.x + width, container_min.y + height);
        const auto   io            = ImGui::GetIO();
        
        ImGui::PushClipRect(container_min, container_max, true);
        ...
        if (ImGui::IsMouseHoveringRect(container_min, container_max)) {
            if (fabsf(io.MouseWheel) > FLT_EPSILON) {
                ...
                min_display_value -= added_value / 2 * offset;
                max_display_value += added_value / 2 * (1 - offset);
                updated = true;
            }
        }
