...
static double width = 1333;
static std::shared_ptr<Material> planet_material = nullptr;

PlanetV2::PlanetV2(const World& in_world) : world(in_world)
{
	root = std::make_shared<PlanetV2Region>(in_world, 10);
	root->regenerate(10, width, 2000, Eigen::Vector3d(0, 0, 0));
}

std::shared_ptr<Material> PlanetV2::get_landscape_material()
{
	if (planet_material)
...
void PlanetV2::tick(double delta_time)
{
...
PlanetV2Region::PlanetV2Region(const World& in_world, uint32_t in_lod_level) :
	world(in_world), lod_level(in_lod_level), mesh(Mesh::create())
{
	if (lod_level != 0)
		child = std::make_shared<PlanetV2Region>(world, lod_level - 1);
}

void PlanetV2Region::regenerate(uint32_t subdivision, double in_width, double inner_radius,
...
void PlanetV2Region::tick(double delta_time)
{
...
void PlanetV2Region::render()
{
	if (child)
		child->render();

	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	PlanetV2::get_landscape_material()->use();
	PlanetV2::get_landscape_material()->set_model_transform(transform);
	mesh->draw();
