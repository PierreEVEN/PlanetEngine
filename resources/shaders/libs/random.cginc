...
vec2 rand2(vec2 st){
    const float u = 53.2978389725796;
    const float v = 3.78309903253987;
    const float w = 654.597894618979;
    const float x = 56.5798984849798;
    const float sizel = 367023.89362;
    st = vec2(dot(st, vec2(u, v)),
              dot(st, vec2(w, x)));
    return -1 + 2 * fract(sin(st) * sizel);
}

// Value noise

float value_noise(float pos) {
...
// Simplex noise using octaves

float simplex_noise(vec2 pos, int octaves, float persistance, float lacunarity) {
    ...
    for (int i = 0; i < octaves; ++i) {
        height += simplex_noise(pos * frequency) * amplitude;
...
float simplex_noise(vec3 pos, int octaves, float persistance, float lacunarity) {
